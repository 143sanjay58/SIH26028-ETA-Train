...
class NvidiaProvider(AIProvider):
    ...
    async def generate(
        ...
        try:
            ...
            logger.error("NVIDIA generate failed", error=str(e))
            raise
    ...
    async def generate_stream(
        ...
        try:
            async with client.stream("POST", "/chat/completions", json=payload, timeout=self.timeout) as response:
                if response.status_code != 200:
                    ...
                    raise httpx.HTTPStatusError(
                        f"NVIDIA stream error: {response.status_code}",
            ...
            logger.error("NVIDIA stream connection error", error=str(e))
            raise
        except httpx.HTTPStatusError as e:
            ...
            self._update_health(False, str(e))
            logger.error("NVIDIA stream error", error=str(e))
            raise
    ...
    async def health_check(self) -> bool:
        try:
            ...
            logger.warning("NVIDIA health check failed", error=str(e))
            return False